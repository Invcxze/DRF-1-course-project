...
def create_product(request):
    ...
    if serializer.is_valid():
        ...
        return Response({"data": {"id": serializer.data["id"],
                                  "message": "Product added"}}, status=201)
    return Response({"error": {'code': 422, "message": "Validation error",
                               "errors": serializer.errors}}, status=422)
...
def get_list_of_products_from_order(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return Response(
                {"error": {"code": 404, "message": "Cart is empty"}},
                status=404)
        order = Order()
        ...
        async_to_sync(channel_layer.group_send)(
            f"user_{request.user.id}",
            {
                ...
                "message": {"event": "order_processed", "details": "Your order is complete"}
            }
        )
        return Response(
            {"data": {"order_id": order.id, "message": "Order is processed"}})
